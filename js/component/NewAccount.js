...
NewAccount.prototype.getView = function(dataParent) {
    ...
    if (this.data !== undefined) {
        if (this.data.original.addressnumber != 0) {
            var promiseAll = [];
            promiseAll.push(moduleDatabase.getModule("streets").load({ WHERE: [{ id: this.data.original.streetid }] }));
            promiseAll.push(moduleDatabase.getModule("wards").load({ WHERE: [{ id: this.data.original.wardid }] }));
            Promise.all(promiseAll).then(function() {
                ...
                address.setAddressData(self.data.original);
            })
        }
    }
    ...
    var selectPermission = _({
        ...
        on: {
            activevaluechange: function(event) {
                            ...
                            child: [{
                                    ...
                                    on: {
                                        click: function(event) {
                                            ...
                                            for (var i = 56; i < 70; i++) {
                                                if ($("div.checkbox_" + i, self.$view).checked == true && $("div.checkbox_" + i, self.$view).disabled !== true) {
...
NewAccount.prototype.resetPermissionChoice = function() {
    // this.selectPermission.activeValue = undefined;
}
...
NewAccount.prototype.setValueNull = function(object) {
    ...
    if (object == "state") {
        if (this.selectPermission.values.indexOf(0) !== -1) {
            if (this.selectPermission.values.length > 0)
                this.selectPermission.activeValue = this.selectPermission.values[0];
        } else {
            this.resetPermission();
            this.resetPermissionChoice();
        }
